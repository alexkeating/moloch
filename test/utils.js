...
const config = {
	...
	contractMethods: {
		changeMethods: ["submit_proposal", "send_applicant_tribute", "submit_vote", "process_proposal", "rage_quit", "abort", "update_delegate_key"],
		viewMethods: ["get_current_period", "get_member_proposal_vote", "has_voting_expired"],
	}
};
...
const serializedCreds = fs.readFileSync(
		`${process.env.HOME}/.near-credentials/${config.networkId}/${config.contractName}.json`
	).toString()
...
async function createAccount(accountId, fundingAmount = config.DEFAULT_NEW_ACCOUNT_AMOUNT, secret) {
...
async function getOrCreateAccount(accountId, fundingAmount = config.DEFAULT_NEW_ACCOUNT_AMOUNT) {
